...
async function normaliserCategories(categories) {
  if (!Array.isArray(categories)) {
    throw new ErreurValidation(
      "Le champ catégories doit être un tableau d'identifiants.",
      400
    );
  }
  ...
  const categoriesExistantes = await Categorie.find({
    _id: { $in: uniques },
  }).select("_id");
  const existantesSet = new Set(
    categoriesExistantes.map((cat) => toStringId(cat._id))
  );
...
export async function createLivre(req, res, next) {
  ...
  if (
    ...
    !auteurs ||
    !isbn ||
    !Array.isArray(auteurs) ||
    auteurs.length === 0
  ) {
    throw new ErreurValidation(
      "Les champs titre, isbn et au moins un auteur sont requis.",
      400
    );
  }

  // Normaliser et valider les catégories si fournies
  const categoriesPayload = Object.prototype.hasOwnProperty.call(
    req.body,
    "categories"
  )
    ? req.body.categories
...
export async function searchLivres(req, res, next) {
  ...
  const auteursMatches = await Auteur.find({
